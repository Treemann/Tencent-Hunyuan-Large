...
class ProfilerCallback(TrainerCallback):
    def __init__(self, start_step=30, end_step=32, log_dir="./log/trainer_profile"):
        self.prof = None
        ...
        self.prof = profiler.profile(
            ...
            schedule=profiler.schedule(wait=max(start_step-1, 0), warmup=1, active=end_step-start_step),
            on_trace_ready=profiler.tensorboard_trace_handler(self.log_dir),
            record_shapes=True,
            profile_memory=True,
            with_stack=True
        )
...
def train():
    ...
    profile_callback = ProfilerCallback(start_step=30, end_step=31, log_dir=os.path.join(training_args.output_dir, 'profile'))

    trainer = Trainer(
        model=model, 
        tokenizer=tokenizer, 
        args=training_args,
        callbacks=[CustomSaveCallback, profile_callback],
        **data_module
    )
